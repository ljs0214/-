...
class autoTrade1 :
    ...
    def get_today_data1(self) :
        ...
        daily_data1['noise'] = abs(daily_data1['open'] - daily_data1['close']) / (daily_data1['high'] - daily_data1['low'])
        # 노이즈 1일 평균
        daily_data1['noise_ma20'] = daily_data1['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data1['range'] = daily_data1['high'] - daily_data1['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data1['targetPrice'] = daily_data1['open'].shift(1) + daily_data1['range'].shift(1) * daily_data1['noise_ma20']
        # 5일 이동평균선
    ...
    def get_today_data2(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data2['noise'] = abs(daily_data2['open'] - daily_data2['close']) / (daily_data2['high'] - daily_data2['low'])
        # 노이즈 1일 평균
        daily_data2['noise_ma20'] = daily_data2['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data2['range'] = daily_data2['high'] - daily_data2['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data2['targetPrice'] = daily_data2['open'].shift(1) + daily_data2['range'].shift(1) * daily_data2['noise_ma20']
        # 5일 이동평균선
    ...
    def get_today_data3(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data3['noise'] = abs(daily_data3['open'] - daily_data3['close']) / (daily_data3['high'] - daily_data3['low'])
        # 노이즈 1일 평균
        daily_data3['noise_ma20'] = daily_data3['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data3['range'] = daily_data3['high'] - daily_data3['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data3['targetPrice'] = daily_data3['open'].shift(1) + daily_data3['range'].shift(1) * daily_data3['noise_ma20']
        # 5일 이동평균선
    ...
    def get_today_data4(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data4['noise'] = abs(daily_data4['open'] - daily_data4['close']) / (daily_data4['high'] - daily_data4['low'])
        # 노이즈 1일 평균
        daily_data4['noise_ma20'] = daily_data4['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data4['range'] = daily_data4['high'] - daily_data4['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data4['targetPrice'] = daily_data4['open'].shift(1) + daily_data4['range'].shift(1) * daily_data4['noise_ma20']
        # 5일 이동평균선
    ...
    def get_today_data5(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data5['noise'] = abs(daily_data5['open'] - daily_data5['close']) / (daily_data5['high'] - daily_data5['low'])
        # 노이즈 1일 평균
        daily_data5['noise_ma50'] = daily_data5['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data5['range'] = daily_data5['high'] - daily_data5['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data5['targetPrice'] = daily_data5['open'].shift(1) + daily_data5['range'].shift(1) * daily_data5['noise_ma50']
        # 5일 이동평균선
    ...
    def get_today_data6(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data6['noise'] = abs(daily_data6['open'] - daily_data6['close']) / (daily_data6['high'] - daily_data6['low'])
        # 노이즈 1일 평균
        daily_data6['noise_ma60'] = daily_data6['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data6['range'] = daily_data6['high'] - daily_data6['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data6['targetPrice'] = daily_data6['open'].shift(1) + daily_data6['range'].shift(1) * daily_data6['noise_ma60']
        # 5일 이동평균선
    ...
    def get_today_data7(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data7['noise'] = abs(daily_data7['open'] - daily_data7['close']) / (daily_data7['high'] - daily_data7['low'])
        # 노이즈 1일 평균
        daily_data7['noise_ma70'] = daily_data7['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data7['range'] = daily_data7['high'] - daily_data7['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data7['targetPrice'] = daily_data7['open'].shift(1) + daily_data7['range'].shift(1) * daily_data7['noise_ma70']
        # 5일 이동평균선
    ...
    def get_today_data8(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data8['noise'] = abs(daily_data8['open'] - daily_data8['close']) / (daily_data8['high'] - daily_data8['low'])
        # 노이즈 1일 평균
        daily_data8['noise_ma80'] = daily_data8['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data8['range'] = daily_data8['high'] - daily_data8['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data8['targetPrice'] = daily_data8['open'].shift(1) + daily_data8['range'].shift(1) * daily_data8['noise_ma80']
        # 5일 이동평균선
    ...
    def get_today_data9(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data9['noise'] = abs(daily_data9['open'] - daily_data9['close']) / (daily_data9['high'] - daily_data9['low'])
        # 노이즈 1일 평균
        daily_data9['noise_ma90'] = daily_data9['noise'].rolling(window=1).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        daily_data9['range'] = daily_data9['high'] - daily_data9['low']
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data9['targetPrice'] = daily_data9['open'].shift(1) + daily_data9['range'].shift(1) * daily_data9['noise_ma90']
        # 5일 이동평균선
    ...
    def get_today_data10(self) :
        ...
        # 노이즈 계산 ( 1- 절대값(시가 - 종가) / (고가 - 저가) )
        daily_data10['noise'] = abs(daily_data10['open'] - daily_data10['close']) / (daily_data10['high'] - daily_data10['low'])
        # 노이즈 1일 평균
        ...
        # 목표매수가 ( 시가 + 변동폭 * K )
        daily_data10['targetPrice'] = daily_data10['open'].shift(1) + daily_data10['range'].shift(1) * daily_data10['noise_ma100']
        # 5일 이동평균선
